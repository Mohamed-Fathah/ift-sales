...
import { RefreshCw, Plus, Search, X, Loader2, IndianRupee } from 'lucide-react'
...
export default function SalesReturnsPage() {
  ...
  // Modal state
  const [invoices,        setInvoices]        = useState<SalesInvoiceOption[]>([])
  const [selectedInvoice, setSelectedInvoice] = useState<SalesInvoiceOption | null>(null)
    ...
    try {
      ...
      if (ids.length > 0) {
        const { data: items } = await supabase
          .from('sales_return_items').select('return_id').in('return_id', ids)
  ...
  const openModal = async () => {
    ...
    const { data } = await supabase
      .from('sales_invoices')
      .select('id, invoice_no, invoice_date, customer_name, customer_phone, location_id')
  ...
  const handleInvoiceChange = async (invoiceId: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('material_id, qty, mrp, rate, materials(title, item_code)')
  ...
  const handleSave = async () => {
    if (!selectedInvoice) return toast.error('Select a sales invoice')
    if (activeItems.length === 0) return toast.error('Enter return qty for at least one item')
    setSaving(true)
    try {
      const supabase = createClient()
      const returnNo = await getNextInvoiceNo('return_sales')
      ...
      // Increment stock (items come back from customer)
        ...
        for (const item of activeItems) {
          ...
          if (stockRow) {
            await supabase.from('stock').update({
              qty_in_hand: Number((stockRow as any).qty_in_hand) + item.qty,
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="page-sub mt-0.5">Goods returned by customers</p>
        </div>
        ...
        <div className="stat-card">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
              ...
              <RefreshCw size={18} style={{ color: 'var(--ift-navy)' }} />
            </div>
            <div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-gray-400">
              <RefreshCw size={36} />
              <p className="text-sm font-medium">No sales returns yet</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">New Sales Return</h3>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              {/* Invoice selector */}
              <div>
                <label className="label">Sales Invoice *</label>
                <select
                  className="input"
                ...
                >
                  ...
                  {invoices.map(inv => (
                    <option key={inv.id} value={inv.id}>
                      {inv.invoice_no} — {inv.customer_name} ({fmtDate(inv.invoice_date)})
                    </option>
                  ))}
                </select>
              </div>
